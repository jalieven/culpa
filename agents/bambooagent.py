...
class BambooAgent:
    def __init__(self, config, redis_repo):
        logging.info("Init component BambooAgent!")
        self.config = config
        self.redis_repo = redis_repo
        self.redis = self.redis_repo.redis
    ...
    def fetchBuildInfo(self, build):
        bamboo = self.config['apiUrl'] + build['key'] + "/latest.json"
        request = urllib2.Request(bamboo)
        base64string = base64.encodestring('%s:%s' % (self.config['username'],
    ...
    def checkBamboo(self):
        ...
        for build in self.config['builds']:
            ...
            if self.redis.hget(build['key'], self.redis_state_key) != build_state:
                self.redis.hset(build['key'], self.redis_state_key, build_state)
                self.redis.hset(build['key'], self.redis_user_key, build_user)
                self.redis.hset(build['key'], self.redis_failed_tests_key, build_failed_tests)
                self.redis.hset(build['key'], self.redis_success_tests_key, build_success_tests)
                build_speech = "Build+" + build[
                    'speak'] + "+" + build_state + "!" + "Brought+to+you+by:" + build_user + "!"
                test_speech = ""
                ...
                if build_state == self.bamboo_failed_value:
                    redis_failed_tests = self.redis.hget(build['key'], self.redis_failed_tests_key)
                    ...
                    redis_success_tests = self.redis.hget(build['key'], self.redis_success_tests_key)
                    if int(redis_success_tests) < int(build_success_tests):
                        ...
                        test_speech = "Hooray!+" + str(
                            more_tests) + "+more+tests!" + "Brought+to+you+by:" + build_user + "!"
                        self.redis.hset(build['key'], self.redis_failed_tests_key, build_failed_tests)
                        self.redis.hset(build['key'], self.redis_success_tests_key, build_success_tests)
                        bunny_speech = build_speech + test_speech
                        print(bunny_speech)
